...
<head>
    ...
    <title>{{ $title ?? 'Owner Dashboard - SaltPro' }}</title>
...
<body class="text-textDark antialiased flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-surface shadow-md flex flex-col h-full z-20">
        ...
        <nav class="flex-1 overflow-y-auto py-4">
            <ul class="space-y-1">
                <li>
                    <a href="{{ route('owner.dashboard') }}" class="sidebar-link flex items-center px-6 py-3 text-sm font-medium text-textLight">
                        <i class="fa-solid fa-chart-pie w-6"></i> Ringkasan Bisnis
                    </a>
                </li>
                <li>
                    <a href="{{ route('owner.reports.index') }}" class="sidebar-link flex items-center px-6 py-3 text-sm font-medium text-textLight">
                        <i class="fa-solid fa-file-invoice-dollar w-6"></i> Laporan Penjualan
                    </a>
                </li>
            </ul>
        </nav>
    ...
    <div class="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header class="h-16 bg-surface shadow-sm flex items-center justify-between px-8 z-10">
            <h1 class="text-xl font-semibold">{{ $header ?? 'Dasbor Pemilik' }}</h1>

            <div class="flex items-center gap-6">
                <div class="flex items-center gap-3 border-l border-gray-200 pl-6">
                    <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'Owner') }}&background=253b70&color=fff" alt="Owner"
                        class="w-9 h-9 rounded-full object-cover">
                    <div class="hidden md:block">
                        <p class="text-sm font-semibold leading-tight">{{ explode(' ', auth()->user()->name ?? 'Owner Toko')[0] }}</p>
                        <p class="text-xs text-primary font-medium">Pemilik Toko</p>
                    </div>
                </div>
            </div>
        </header>
